...
class Deploy:
    ...
    def start_workflows_k8s(self):
        ...
        if self.workflows_user is not None:
            for wflow_user in tqdm(self.workflows_user):
                logging.info(f"[++] Starting workflow: [{wflow_user['name']}].")
                containers, tracker_ctn = self.start_workflow_k8s(wflow_user)
                logging.info(f"[+] Workflow: [{wflow_user['name']}] was started successfully.")
                # for w in self.workflows:
                #     if w['name'] == wflow_user['name']:
                #         for node in w['nodes']:
    ...
    def start_workflow_k8s(self, workflow):

        config.load_kube_config("/gpfs/bsc_home/xpliu/.kube/config")

        if 'tracker' in workflow.keys():
           with open(path.join(path.dirname(__file__), f"tracker-{workflow['name']}"+"-deployment.yaml")) as f:
             dep = yaml.safe_load(f)
             k8s_apps_v1 = client.AppsV1Api()
             resp = k8s_apps_v1.create_namespaced_deployment(
                 body=dep, namespace="default")
             logging.info(f"Deployment created. status='%s'" % resp.metadata.name)

        for wflow in workflow['executors']:

            logging.info(f"[+] Starting pods: [{workflow['name']}:{wflow['name']}].")
...
#                 logging.info(f"Deployment created. status='%s'" % resp.metadata.name)
#            else:
            with open(path.join(path.dirname(__file__), f"{wflow['name']}-deployment.yaml")) as f:
               dep = yaml.safe_load(f)
               k8s_apps_v1 = client.AppsV1Api()
               resp = k8s_apps_v1.create_namespaced_deployment(
                   body=dep, namespace="default")
    ...
    def run_workflows_k8s(self):
        ...
        for wf_user in tqdm(self.workflows_user):
            ...
            if 'parallel' in wf_user.keys():
                logging.info(f"[+] Workflow: [{wf_user['name']}] was run in parallel.")
                environments = self.run_workflow_k8s(wf_user, wf_user['parallel'])
            else:
                environments = self.run_workflow_k8s(wf_user)
    ...
    def run_workflow_k8s(self, workflow, parallel=False):
        """
        ...
        if parallel:
            ...
            for step in workflow['executors']:
                ...
                ret = v1.list_namespaced_pod(namespace='default', label_selector="app", watch=False)
 
                ipaddress = None
                for item in ret.items:
                   if item.metadata.labels['app'] == f"tracker-{workflow['name']}":
                       ipaddress = item.status.pod_ip

                for item in ret.items: 
                   if item.metadata.labels['app'] == f"{step['name']}":
                       ...
                       try:
                           resp = core_v1.read_namespaced_pod(name=name,
                               ...
                               logging.info(f"ERROR no pod ")
                               exit(1)

                       if 'parameters' in step.keys():
                           exec_command = ['/bin/sh']
                           commands = [ 'cd /app/workflow',
                                   'pwd',
                                   f"export MLFLOW_TRACKING_URI=http://{ipaddress}:8001", 
                                   f"python {step['file']} {tools.format_parameters(step['parameters'])}"]
                       ...
                       #logging.info(f"[+] Output:  {resp.read_stderr()} ")

                       logging.info(f"[+] Environment [{workflow['name']}:{step['name']}] finished successfully. ")


        return [None]

    def predict_k8s(self, input_name, port=5001):
        """
        ...
        ipaddress = None
        for item in ret.items:
           if item.metadata.labels['app'] == f"predictor":
               ipaddress = item.status.pod_ip
        
        url = f'http://{ipaddress}:{port}/invocations'
        
                
        logging.info(f"[+] Predictor API at {url}. ")
    ...
    def build_predictor_k8s(self, model_path, image='predictor', name='predictor'):
        """
...
#        # self.logs_run_ctn = logs_run_ctn
    ...
    def run_predictor_k8s(self, image='predictor', name='predictor', port=5001):
        """
        ...
        with open(path.join(path.dirname(__file__), f"predictor-deployment.yaml")) as f:
               dep = yaml.safe_load(f)
               k8s_apps_v1 = client.AppsV1Api()
               resp = k8s_apps_v1.create_namespaced_deployment(
                   body=dep, namespace="default")
        ...
        logging.info(f"[+] Predictor API at [http://ipaddress:{port}]. ")
